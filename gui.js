...
IDE_Morph.prototype.init = function (isAutoFill) {
	...
	this.currentSubsystem = 'farm ops';
	...
	this.subsystemBar = null;
	this.cropSubsystemEditor = null;
	this.soilSubsystemEditor = null;
...
IDE_Morph.prototype.buildPanes = function () {
	...
	this.createCropSubsystemEditor();
	this.createSoilSubsystemEditor();
};
...
IDE_Morph.prototype.createModelBar = function() {
	// assumes the logo has already been created
	var tabCorner = 5,
		tabPadding = 6,
    ...
    if (this.ModelBar) {
        this.ModelBar.destroy();
    }

    this.ModelBar= new Morph();
    this.ModelBar.color = this.frameColor;
    this.add(this.ModelBar);
    ...
    tab = new TabMorph(
        tabColors,
        null, // target
        function () {tabBar.tabTo('farm'); },
        localize('Farm'), // label
        function () {  // query
            return myself.currentTab === 'farm';
        }
    );
    ...
    tab = new TabMorph(
        tabColors,
        null, // target
        function () {tabBar.tabTo('crops'); },
        localize('Crops'), // label
        function () {  // query
            return myself.currentTab === 'crops';
        }
    );
    tab.padding = tabPadding;
    tab.corner = tabCorner;
    ...
    tab = new TabMorph(
        tabColors,
        null, // target
        function () {tabBar.tabTo('soils'); },
        localize('Soils'), // label
        function () {  // query
            return myself.currentTab === 'soils';
        }
    );
    tab.padding = tabPadding;
    tab.corner = tabCorner;
    ...
    tab = new TabMorph(
        tabColors,
        null, // target
        function () {tabBar.tabTo('weather'); },
        localize('Weather'), // label
        function () {  // query
            return myself.currentTab === 'weather';
        }
    );
    tab.padding = tabPadding;
    tab.corner = tabCorner;
    ...
    this.ModelBar.tabBar = tabBar;
    this.ModelBar.add(this.ModelBar.tabBar);

    this.ModelBar.fixLayout = function () {
...
IDE_Morph.prototype.createSubsystems = function () {
    // assumes the logo has already been created
    var myself = this;

    if (this.subsystemBar) {
        this.subsystemBar.destroy();
    }
    this.subsystemBar = new Morph();
    this.subsystemBar.color = this.groupColor;
	this.subsystemBar.setPosition(this.logo.bottomLeft());
    this.subsystemBar.silentSetWidth(this.width());

    function addSubsystemButton(subsystem) {
        var labelWidth = 75,
            colors = [
                myself.frameColor,
        ...
        button = new ToggleButtonMorph(
            ...
            myself, // the IDE is the target
            function () {
                myself.currentSubsystem = subsystem;
                myself.subsystemBar.children.forEach(function (each) {
                    each.refresh();
                });
                //myself.refreshPalette(true);
				// TODO: Refresh stuff
				myself.displaySubsystemEditor();
            },
            subsystem[0].toUpperCase().concat(subsystem.slice(1)), // label
            function () {  // query
                return myself.currentSubsystem === subsystem;
            },
            null, // env
            null, // hint
            null, // template cache
        ...
        button.fixLayout();
        button.refresh();
        myself.subsystemBar.add(button);
        return button;
    }

    function fixSubsystemBarLayout() {
        var buttonHeight = myself.subsystemBar.children[0].height(),
			border = 3,
            xPadding = 2,
            yPadding = 2,
            l = myself.subsystemBar.left(),
            t = myself.subsystemBar.top(),
            i = 0;
		myself.subsystemBar.setWidth(
			myself.width()
		);
		
        myself.subsystemBar.children.forEach(function (button) {
			i += 1;
			if(l + border + button.width() > myself.subsystemBar.width()) {
			...
			l += myself.subsystemBar.children[i-1].width() + 2 * border;
        });

        myself.subsystemBar.setHeight(
				t + buttonHeight + 2 * border - myself.subsystemBar.top()
        );
    }

	// TODO: Subsystems should probably be moved!
	['farm ops', 'crops', 'soils', 'weather', 'equipment', 'markets'].forEach(function(subsystem) {
		addSubsystemButton(subsystem);
	});
    fixSubsystemBarLayout();
	this.subsystemBar.fixLayout = function() {
		fixSubsystemBarLayout();
	};
    this.add(this.subsystemBar);
};

IDE_Morph.prototype.createCropSubsystemEditor = function() {
	// assumes subsystemBar has already been created
	var myself = this,
		stage;
	
	if(this.cropSubsystemEditor) {
...
IDE_Morph.prototype.createSoilSubsystemEditor = function() {
	// assumes subsystemBar has already been created
	var myself = this;
	
	if(this.soilSubsystemEditor) {
		this.soilSubsystemEditor.destroy();
	}
	
	this.soilSubsystemEditor = new Morph();
	this.soilSubsystemEditor.color = new Color(244,244,20);
	
	this.soilSubsystemEditor.fixLayout = function() {
		myself.soilSubsystemEditor.setPosition(myself.subsystemBar.bottomLeft());
		myself.soilSubsystemEditor.setWidth(myself.width());
		myself.soilSubsystemEditor.setHeight(
			myself.height() - myself.subsystemBar.bottom()
		);
	}
	
	this.add(this.soilSubsystemEditor);
	if(this.currentSubsystem !== 'soils') this.soilSubsystemEditor.hide();
};

IDE_Morph.prototype.createWeatherSubsystemEditor = function() {
...
IDE_Morph.prototype.fixLayout = function (situation) {
	...
	// ModelBar
	/*this.ModelBar.setPosition(this.logo.bottomLeft());
	this.ModelBar.setExtent(new Point(
		this.width(),
		this.logo.height()
	));
	this.ModelBar.fixLayout();
	*/
/* NEED TO REDO FOR NEW LAYOUT
...
IDE_Morph.prototype.displaySubsystemEditor = function() {
	this.cropSubsystemEditor.hide();
	this.soilSubsystemEditor.hide();
	switch(this.currentSubsystem) {
		case 'crops':
			this.cropSubsystemEditor.show();
			break;
		case 'soils':
			this.soilSubsystemEditor.show();
			break;
	}
};
